    ...
    public partial class frmViewRefunds : Form
    {
        ...
        private void btnview_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Get detailed refund items for this order
                string query = $"SELECT products, quantity, unitprice, reason, daterefunded, timerefunded, refundedby FROM tblrefunds WHERE orderid = '{orderID.Replace("'", "''")}'";
                DataTable refundDetails = db.GetData(query);
                ...
                // Show refund details in a dialog
                ShowRefundDetails(orderID, refundDetails);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error viewing refund details: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void ShowRefundDetails(string orderID, DataTable refundDetails)
        {
            Form detailsForm = new Form();
            detailsForm.Text = $"Refund Details - Order: {orderID}";
            detailsForm.Size = new Size(1000, 650);
            ...
            detailsForm.BackColor = Color.FromArgb(250, 250, 250);

            // Pagination variables
            int currentPage = 1;
            int pageSize = 5;
            int totalPages = 0;
            int totalItems = refundDetails.Rows.Count;
            ...
            lblOrder.Font = new Font("Segoe UI", 11F, FontStyle.Regular);
            lblOrder.ForeColor = Color.White;
            lblOrder.Dock = DockStyle.Right;
            lblOrder.TextAlign = ContentAlignment.MiddleRight;

            headerPanel.Controls.Add(lblOrder);
            headerPanel.Controls.Add(lblTitle);

            // Content Panel
            Panel contentPanel = new Panel();
            contentPanel.Dock = DockStyle.Fill;
            contentPanel.Padding = new Padding(0, 10, 0, 10);
            ...
            dgvDetails.GridColor = Color.FromArgb(230, 230, 230);
            dgvDetails.ColumnHeadersHeight = 40;
            dgvDetails.RowTemplate.Height = 35;
            dgvDetails.ScrollBars = ScrollBars.Both; // Allow scrolling if needed
            ...
            // Create columns
            string[] columns = new string[] { "Product", "Quantity", "Unit Price", "Total Amount", "Reason", "Date", "Time", "Refunded By" };
            int[] widths = new int[] { 200, 60, 100, 120, 180, 90, 80, 120 };
            ...
            for (int i = 0; i < columns.Length; i++)
            {
                ...
                else if (columns[i] == "Unit Price" || columns[i] == "Total Amount")
                {
            ...
            // Create a container panel for centering the pagination controls
            Panel paginationContainer = new Panel();
            paginationContainer.Size = new Size(600, 50);
            paginationContainer.BackColor = Color.Transparent;
            ...
            lblPageInfo.Size = new Size(200, 35);
            lblPageInfo.Text = $"Page 1 of {totalPages}";

            // Position pagination controls in the center
            btnPrev.Location = new Point(50, 10);
            lblPageInfo.Location = new Point(200, 10);
            btnNext.Location = new Point(450, 10);

            paginationContainer.Controls.AddRange(new Control[] { btnPrev, lblPageInfo, btnNext });

            // Center the pagination container
            paginationContainer.Location = new Point(
                (paginationPanel.Width - paginationContainer.Width) / 2,
                (paginationPanel.Height - paginationContainer.Height) / 2
            );
            paginationContainer.Anchor = AnchorStyles.None;

            paginationPanel.Controls.Add(paginationContainer);
            ...
            foreach (DataRow row in refundDetails.Rows)
            {
                ...
                if (int.TryParse(quantityStr, out int quantity) && decimal.TryParse(unitPriceStr, out decimal unitPrice))
                {
                    totalRefundAmount += quantity * unitPrice;
                }
            }
            ...
            btnClose.Location = new Point(
                (actionPanel.Width - btnClose.Width) / 2,
                (actionPanel.Height - btnClose.Height) / 2
            );
            btnClose.Anchor = AnchorStyles.None;

            actionPanel.Controls.Add(btnClose);

            // Method to load current page
            void LoadCurrentPage()
            {
                ...
                for (int i = startIndex; i < endIndex; i++)
                {
                    DataRow row = refundDetails.Rows[i];

                    string product = row["products"]?.ToString() ?? "";
                    string quantityStr = row["quantity"]?.ToString() ?? "0";
                    string unitPriceStr = row["unitprice"]?.ToString() ?? "0";
                    string reason = row["reason"]?.ToString() ?? "";
                    string date = row["daterefunded"]?.ToString() ?? "";
                    string time = row["timerefunded"]?.ToString() ?? "";
                    string refundedBy = row["refundedby"]?.ToString() ?? "";

                    // Calculate line total with proper variable initialization
                    int quantity = 0;
                    decimal unitPrice = 0;
                    decimal lineTotal = 0;

                    if (int.TryParse(quantityStr, out quantity) && decimal.TryParse(unitPriceStr, out unitPrice))
                    {
                        lineTotal = quantity * unitPrice;
                    }

                    dgvDetails.Rows.Add(product, quantity, unitPrice, lineTotal, reason, date, time, refundedBy);
                }
